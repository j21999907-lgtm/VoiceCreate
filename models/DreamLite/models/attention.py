...
from diffusers.models.embeddings import SinusoidalPositionalEmbedding, FluxPosEmbed
...
class BasicTransformerBlock(nn.Module):
    ...
    Parameters:
        ...
        use_self_attention (`bool` *optional*, defaults to True):
            Whether to use self-attention layers. In this case no self attention layers are used.
    """
    ...
    def __init__(
        ...
        elif positional_embeddings == 'rope': # TODO；待实现ROPE
            axes_dims_rope = [1, 2, 3, 4]
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            # print(f'ca:{num_kv_heads}, {cross_attention_dim}')
            self.attn2 = Attention(
                query_dim=dim,
    ...
    def forward(
        ...
        if self.pos_embed is not None:
            ...
            # image_rotary_emb = self.pos_embed(ids) # 基于文本和图片idx得到位置编码，对于UNet应该只需要图片，att_forward需要传入
        # 1. Prepare GLIGEN inputs
        cross_attention_kwargs = cross_attention_kwargs.copy() if cross_attention_kwargs is not None else {}
        gligen_kwargs = cross_attention_kwargs.pop("gligen", None)

        if self.use_self_attention:
            # 如果存在image_rotary_emb
