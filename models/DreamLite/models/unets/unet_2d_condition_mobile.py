...
from diffusers.models.transformers.transformer_hunyuan_video import HunyuanVideoTokenRefiner
...
class DreamLiteUNetModel(
    ...
    def _set_encoder_hid_proj(
        ...
        if encoder_hid_dim_type == "text_proj":
            ...
            assert cross_attention_dim % 128 == 0, "`cross_attention_dim` has to be divisible by 128."
    ...
    def get_aug_embed(
        ...
        if self.config.addition_embed_type == "text":
            ...
            time_ids = added_cond_kwargs.get("time_ids")
            time_embeds = self.add_time_proj(time_ids.flatten())
            time_embeds = time_embeds.reshape((-1, self.projection_class_embeddings_input_dim))
    ...
    def process_encoder_hidden_states(
        self, encoder_hidden_states: torch.Tensor, timestep: Union[torch.Tensor, float, int], encoder_attention_mask: Optional[torch.Tensor], added_cond_kwargs: Dict[str, Any]
    ) -> torch.Tensor:
        if self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_proj":
            encoder_hidden_states = self.encoder_hid_proj(encoder_hidden_states)
        elif self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_proj_rms":
            encoder_hidden_states = self.encoder_hid_proj(encoder_hidden_states)
        elif self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_token_refiner":
            encoder_hidden_states = self.encoder_hid_proj(encoder_hidden_states, timestep, encoder_attention_mask)
        elif self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "large_text_token_refiner":
            encoder_hidden_states = self.encoder_hid_proj(encoder_hidden_states, timestep, encoder_attention_mask)
    ...
    def forward(
        ...
        encoder_hidden_states = self.process_encoder_hidden_states(
            encoder_hidden_states=encoder_hidden_states, timestep=timestep, encoder_attention_mask=encoder_attention_mask, added_cond_kwargs=added_cond_kwargs
        )
